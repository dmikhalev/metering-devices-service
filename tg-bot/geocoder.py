...
path = "http://f6d74aebbc41.ngrok.io"


def get_my_news():
    add = "/api/v1/info/news/7"
    res = requests.get(path + add).json()
    return [i['text'] for i in res]


def get_last_check(username):
    add = "/api/v1/pdf_receipt/30"
    headers = {'Content-type': 'application/json', 'Accept': 'application/json'}
    res = requests.post(path + add, headers=headers, json={'username': username}).json()
    return res['pdfs']
